// Uptime check for steepd. Runs on a 5-minute cron, fetches /healthz (retrying once), and emails the
...
export default {
  async scheduled(event, env, ctx) {
    ...
    let low = false;
    let detail = "";
    // Two attempts, a few seconds apart. A single probe lost between Cloudflare's edge and
    // Railway's is not an outage, and one such loss has already paged the operator once.
    for (let attempt = 1; attempt <= 2 && !healthy; attempt++) {
      if (attempt > 1) await new Promise((resolve) => setTimeout(resolve, 3_000));
      try {
    ...
    if (!healthy) detail += " (after 2 attempts)";
